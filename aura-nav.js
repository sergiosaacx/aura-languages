(function() {
  ...
  var CSS = [
    ...
    '.aura-sl-logo{width:34px;height:34px;display:flex;align-items:center;justify-content:center;color:#c4ff3d;font-family:monospace;font-size:1.4rem;font-weight:900;margin-bottom:14px;cursor:pointer;flex-shrink:0;}',
  ...
  function slBtn(id, label, svg, dest, key) {
    var cls = 'aura-sl-btn' + (leftActive === key ? ' active' : '');
    var oc  = dest ? ' onclick="auraNav(\'' + dest + '\')"' : '';
    return '<button class="' + cls + '"' + oc + '>'
      + '<svg viewBox="0 0 24 24">' + svg + '</svg>'
      + '<span class="aura-sl-lbl">' + label + '</span>'
      + '</button>';
  }

  function srBtn(id, label, svg, oc, key) {
    var cls = 'aura-sr-c' + (rightActive === key ? ' aura-sr-active' : '');
    var click = oc ? ' onclick="' + oc + '"' : '';
    return '<button class="' + cls + '"' + click + ' title="' + label + '">'
      + '<svg viewBox="0 0 24 24">' + svg + '</svg>'
  ...
  var D = {
    ...
    rank:      '<path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/>',
    comu:      '<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/>',
    ...
    shadow:    '<path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/>',
  ...
  nav.innerHTML =
    ...
    slBtn('home',     'Home',         D.home,     'home.html',      'home') +
    slBtn('dash',     'Dashboard',    D.dash,     'dashboard.html', 'dashboard') +
    slBtn('rank',     'Ranking',      D.rank,     '',               'ranking') +
    slBtn('comu',     'Comunidad',    D.comu,     '',               'comunidad') +
    slBtn('tienda',   'Tienda',       D.tienda,   'tienda.html',    'tienda') +
    '<div class="aura-sl-spacer"></div>' +
    slBtn('settings', 'Configuracion',D.settings, 'settings.html',  'settings');
  ...
  srTop.style.flex = '1';
  srTop.innerHTML =
    srBtn('mv', 'Movies',      D.movies, "auraNav('movies.html')",      'movies') +
    srBtn('ll', 'LyricLab',    D.lyric,  "auraNav('lyriclab.html')",    'lyriclab') +
    srBtn('sh', 'ShadowLab',   D.shadow, "auraNav('shadowlab.html')",   'shadowlab') +
    srBtn('fc', 'Flashcards',  D.flash,  "auraNav('flashcards.html')",  'flashcards') +
    srBtn('co', 'Collocations',D.colloc, "auraNav('collocations.html')","collocations") +
    srBtn('so', 'Social',      D.social, '',                            '') +
    '<div class="aura-sr-div"></div>' +
    '<button class="aura-sr-c aura-logout" onclick="auraLogout()" title="Cerrar sesion">'
    + '<svg viewBox="0 0 24 24">' + D.logout + '</svg>'
    + '<span class="aura-sr-lbl">Cerrar sesion</span>'
    + '</button>';

  var srBot = document.createElement('div');
  srBot.className = 'aura-sr';
  srBot.innerHTML =
    srBtn('ch', 'Chat',         D.chat,    'if(window.openAuraChat)openAuraChat()',       '') +
    srBtn('tc', 'Teacher',      D.teacher, '',                                            '') +
    srBtn('fr', 'Agregar amigo',D.friend,  'if(window.openAuraFriends)openAuraFriends()', '');
  ...
  window.openAuraChat = window.openAuraChat || function() { if (window._auraFriendsOpen) window._auraFriendsOpen(); };
  window.openAuraFriends = window.openAuraFriends || function() { if (window._auraFriendsOpen) window._auraFriendsOpen(); };
