...
window.initHeroSlider = function(aura) {
    ...
    // ── Herramientas — solo las del idioma activo ────────────────────────────
    aura.sb.from('home_tools').select('*')
      ...
      .then(function(tw) {
        ...
        if (!tw.data || !tw.data.length) return; // sin config para este idioma → HTML estático
    ...
    // ── Novedades — solo las del idioma activo ───────────────────────────────
    aura.sb.from('novedades').select('*')
      ...
      .then(function(nv) {
        ...
        if (!items.length) return; // sin novedades para este idioma → HTML estático
