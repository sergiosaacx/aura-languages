...
function updateNextButton() {
  ...
  if (!allFilled) {
    GAME.completedRound = false;
    if(window.auraT) $('tipText').innerHTML = window.auraT('coll_select_hint'); else $('tipText').innerHTML = 'Selecciona las palabras correctas del banco para completar la frase.';
  }
  setActiveSlot();
}
...
function _colDiffUpdateCounts() {
  ...
  if (sub){ sub.textContent = PHRASES.length + ' ' + (window.auraT ? window.auraT('coll_ui_count') : 'colocaciones · 4 niveles'); }
}
